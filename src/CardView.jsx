...
import { getProblemStatus, setProblemStatus, problemStatus, getCardPosition, setCardPosition, setCurrentProblem } from './utils/storageUtils';

const CardView = forwardRef(({ filteredProblems }, ref) => {
  ...
  // Track current problem for syncing
  useEffect(() => {
    if (currentProblem) {
      setCurrentProblem(
        currentProblem.categoryName,
        currentProblem.name,
        currentProblem.categoryIndex,
  ...
  useImperativeHandle(ref, () => ({
    navigateToProblem: (categoryName, problemName) => {
      const index = displayProblems.findIndex(
        p => p.categoryName === categoryName && p.name === problemName
      );
      if (index !== -1) {
      ...
      <div className="w-full max-w-2xl">
        ...
        >
          ...
          <div className="flex gap-2 sm:gap-3 mb-6 flex-wrap">
            {currentProblem.takeuforwardLink && (
              <a
                href={currentProblem.takeuforwardLink}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center p-2 sm:px-4 sm:py-3 bg-gray-800 hover:bg-gray-700 active:bg-gray-600 text-white rounded-lg transition-all font-medium shadow-lg border border-gray-700"
              ...
              >
                ...
                <span className="hidden sm:inline ml-2">Article</span>
              </a>
            )}
            {currentProblem.youtubeLink && (
              <a
